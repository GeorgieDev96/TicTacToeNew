function init() {
    ...
    let currentPlayer = 'playerJ';

    // Looping though all the board items
    items.forEach(item => {
        item.addEventListener('click', (e) => {
            // player Move
            const index = gridArray.indexOf(e.target);
            if (
                items[index].classList.contains('playerJ') ||
                items[index].classList.contains('computer')
            ) {
                return;
            }

            items[index].classList.add('playerJ');

            // splicing the move fomr the tracking list
            const spliceNr = tracking.indexOf(index + 1)
            tracking.splice(spliceNr, 1);

            // win check for the player
            if (winCheck('playerJ', items)) {
                playerTitle.innerHTML = "playerJ Win!!"
                document.body.classList.add('over');
            }

            // check for draw
            if (tracking.length === 0) {
                playerTitle.innerHTML = " Again, it's a Draw";
                document.body.classList.add('over');
                return;
            }
